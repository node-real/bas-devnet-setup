...
func simulateSystemContract(genesis *core.Genesis, systemContract common.Address, rawArtifact []byte, constructor []byte, balance *big.Int) error {
	...
	if err != nil {
		fmt.Printf("Error. CreateWithAddress:%s\n", err.Error())
...
type genesisConfig struct {
	AutoCreate         autoCreate                `json:"autoCreate"`
	...
	InitialOwnerStakes map[common.Address]string `json:"initialOwnerStakes"`
}
...
func createValidators(count int, pwd string, dir string) []common.Address {
	fmt.Printf("count:%d pwd:%s dir:%s\n", count, pwd, dir)
	var validators []common.Address
	for i := 0; i < count; i++ {
		account, err := keystore.StoreKey(dir, pwd, keystore.StandardScryptN, keystore.StandardScryptP)
		if err != nil {
			fmt.Errorf("create validator address error:%v\n", err)
			panic(err)
		}
		validators = append(validators, account.Address)
		fmt.Printf("index :%d url:%s:%s, address:%s\n", i, account.URL.Scheme, account.URL.Path, account.Address.Hex())
	}
	return validators
}
